...
from app.config import GEMINI_API_KEY, GEMINI_MODEL, GEMINI_FALLBACK_MODEL, SYSTEM_PROMPT
from app.schemas import ChatRequest, ChatResponse
from app.providers.base import LLMProvider
from app.tools.registry import TOOL_DEFINITIONS, TOOL_DISPATCH
...
def _convert_schema_types(schema: dict) -> dict:
    ...
    if "properties" in converted:
        converted["properties"] = {
            k: _convert_schema_types(v) for k, v in converted["properties"].items()
        }
    if "items" in converted:
        converted["items"] = _convert_schema_types(converted["items"])
    return converted
...
class GeminiProvider(LLMProvider):
    ...
    def _run_chat_session(self, model_name: str, req: ChatRequest):
        """Runs one full chat+tool-calling exchange against a specific model."""
        config_kwargs = {
            "system_instruction": SYSTEM_PROMPT,
            "temperature": req.temperature,
            "top_p": req.top_p,
        ...
        config = types.GenerateContentConfig(**config_kwargs)
        chat = self._client.chats.create(model=model_name, config=config)
    ...
    async def chat(self, req: ChatRequest) -> ChatResponse:
        ...
        try:
            reply, tool_calls_log = self._run_chat_session(GEMINI_MODEL, req)
            model_used = GEMINI_MODEL
        except ClientError as e:
            # Only downgrade on rate-limit/quota errors (HTTP 429), not on
            # unrelated errors like bad request formatting (400).
            if e.code == 429 and GEMINI_FALLBACK_MODEL:
                reply, tool_calls_log = self._run_chat_session(GEMINI_FALLBACK_MODEL, req)
